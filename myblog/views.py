...
def search(req):
    ...
    diary = Diary.objects.filter(title__contains=keyword).order_by("-update_date").values("title","description","article_id","update_date","type")
    study_diary = StudyDiary.objects.filter(title__contains=keyword).order_by("-update_date").values("title","description","article_id","update_date","type")
    article = Article.objects.filter(title__contains=keyword).order_by("-update_date").values("article_id","title","description","update_date","author__name","type")
    for d in diary:
        if d:
            data_list.append(d)
        else:
